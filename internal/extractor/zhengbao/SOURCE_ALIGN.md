...
| Direct courseware fallback. | Courseware metadata can already contain direct media/material URLs in fixture or doorman payloads. | `direct.go` now extracts direct `videoUrl`/`m3u8Url`/`mp4Url` and document/resource URL fields when directory/material pages are unavailable. |
